...
const POLL_INTERVAL = 15000 // 15 seconds - peaceful, not frantic
...
export function useWebSocket() {
  ...
  const fetchRounds = useCallback(async () => {
    try {
      ...
      setConnectionStatus(newCount > 0 ? `+${newCount} flights` : 'Live')

      // Clear the "new" highlight after a peaceful moment
      if (newCount > 0) {
        setTimeout(() => {
          setRounds(prev => prev.map(r => ({ ...r, isNew: false })))
          setConnectionStatus('Live')
        }, 3000) // Longer fade for peaceful atmosphere
      }
    } catch (e) {
      console.error('Failed to fetch flights:', e)
      setConnected(false)
      setConnectionStatus('Grounded')
    }
  }, [])
