"""
Skyward Data Collector

Collects crash multiplier data from BGaming Skyward demo.
...
DATABASE_PATH: str = os.getenv("DATABASE_PATH", "skyward.db")
...
MULTIPLIER_FIELDS: List[str] = [
    ...
    'crashMultiplier', 'odds', 'coefficient'
]
...
# =============================================================================
# Skyward Collector Class
# =============================================================================

class SkywardCollector:
    """
    Collects crash game data from BGaming Skyward using Playwright.
    ...
    Example:
        collector = SkywardCollector()
        await collector.run(demo_url="https://demo.bgaming.com/skyward")
    """
    ...
    def __init__(
        ...
        """
        Initialize the Skyward collector.
        ...
        logger.info(f"SkywardCollector initialized with db_path={self.db_path}")

    async def init_db(self) -> None:
        """
        Initialize the database schema.

        Creates the skyward_rounds table if it doesn't exist.
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute("""
                    CREATE TABLE IF NOT EXISTS skyward_rounds (
                ...
                """)
                await db.execute(
                    "CREATE INDEX IF NOT EXISTS idx_created ON skyward_rounds(created_at DESC)"
                )
                await db.execute(
                    "CREATE INDEX IF NOT EXISTS idx_multiplier ON skyward_rounds(crash_multiplier)"
                )
                await db.commit()
    ...
    async def save_round(self, round_id: str, multiplier: float) -> bool:
        """
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    "INSERT OR IGNORE INTO skyward_rounds (round_id, crash_multiplier) VALUES (?, ?)",
                    (round_id, multiplier)
                )
                await db.commit()
                self.rounds_collected += 1
                logger.info(
                    f"Round saved: round_id={round_id}, multiplier={multiplier:.2f}x, "
    ...
    def parse_ws_message(self, data: Union[Dict[str, Any], List, Any]) -> None:
        ...
        if round_id is None:
            ...
            round_id = f"skyward_{datetime.now().timestamp()}"
            logger.debug(f"Generated round_id: {round_id}")

        # Save the round asynchronously
        asyncio.create_task(self.save_round(str(round_id), float(multiplier)))
    ...
    async def generate_test_data(self, count: int = 100) -> None:
        ...
        for i in range(count):
            ...
            try:
                await self.save_round(round_id, multiplier)
    ...
    async def run(
        ...
        """
        logger.info("Starting Skyward collector...")
...
async def main() -> None:
    ...
    if test_mode:
        ...
        demo_url = os.getenv("DEMO_URL", "https://demo.bgaming.com/skyward")
